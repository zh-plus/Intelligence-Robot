...
x(k-1)=-0.75; % initilize the state x
y(k-1)=-0.75; % initilize the state y
...
%=====================================
%% =========== The main loop ==========
delta_x = objective_x-x(k-1);
delta_y = objective_y-y(k-1);
...
while(abs(rho) > tolerance)
    t=t+T; % increase the time

    v_rho = -k_rho * rho * cos(alpha)
    v_alpha = k_rho * sin(alpha) - k_alpha * alpha - k_beta * beta;
    v_beta = -k_rho * sin(alpha);
...
%% === Draw the mobile robot & Path ====
    function draw_robot()
        xmin=-1.2 * 1; % setting the figure limits
        xmax=1.2 * 1;
        ymin=-1.2 * 1;
        ymax=1.2 * 1;
